...
export default function ProfileForm() {
  ...
  async function onSubmit(values: FormData, e: React.BaseSyntheticEvent) {
    ...
    if (qData && origData && editMode) {
      ...
      const changedAnswers = values.answers.flatMap(
        (
          ans: {
            types: {
              type: string;
          ...
          const changedTypes = ans.types
            ...
            .filter((t): t is typeof t => t !== undefined) as {
            type: string;
            text: string;
            id: string;
            reference?: string;
          ...
          if (changedTypes.length > 0) {
            ...
            const changedSurah = ans.s_id !== origAns[i].s_id;
            if(changedSurah){

              return [{s_id:ans.s_id, id: ans.id, types: changedTypes }];
            }
    ...
    if (action === "upload") {
      const result = await upload({
        topicId: values.topic.id as Id<"Topics">,
        qId: values.question.id as Id<"Questions">,
  ...
  const findLabel = ({
    ...
    if (editMode) {
      ...
      if (
        ...
        console.log(id," s id")
        setValue(`answers.${answerIndex}.s_id`, id);
      }
                ...
                <FormItem className="h-fit py-10 ">
                  ...
                  <FormControl>
                    <Input
                      ...
                      onChange={(e) =>
                        findLabel &&
                        findLabel({ type: "Question", text: e.target.value })
                      }
                    />
                  </FormControl>
                </FormItem>
              ...
              <AlertDialog>
                <AlertDialogTrigger>
                  <Button
                    type="button"
                    onClick={() =>
                      delAll({ id: origData?.question.id as Id<"Questions"> })
                    }
                    variant={"destructive"}
                  >
                    Delete All
